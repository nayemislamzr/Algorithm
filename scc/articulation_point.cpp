                         ...
                         vector<int>& low, vector<int>& parent, vector<bool>& ap)
{
    ...
    for(auto& v : adj[node])
    {
        if(discovery_time[v] == -1) // not visited
        {
            ...
            if(parent[node] == -1 && child > 1) // rooted node having more than 1 subgraph
                ap[node] = true;
            else if(parent[node] != -1 && low[v] >= discovery_time[node]) // non rooted, at least 1 child that does not having
                ap[node] = true; // backedge back to ancestor of the "node"
        }
